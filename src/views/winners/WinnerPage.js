...
export default function Winner({props}) {
  ...
  const handleSearch = () => {
    setSearch(true);
    const filteredList = winnerListByLottery
      .filter((item) => (lotteryIdVal ? item.lotteryId === lotteryIdVal : true))
      .map((item) => ({
        ...
        data: item.data
          .filter((date) => {
            const selectedDate = dateVal
              ...
              : null;
            return !dateVal || selectedDate === dateVal;
          })
          .filter((number) => {
            return !numberVal || number.ticketNumber === numberVal;
          }),
      }))
      .filter((item) => item.data.length > 0);
    setSearchWinnerListByLottery(filteredList);
  };
        ...
        <div className="container">
          ...
          <div className="row d-flex justify-content-center align-items-center mt-4">
            <div className="col-lg-12">
              {(
                search
                  ? searchWinnerListByLottery.length > 0
                  : winnerListByLottery.length > 0
              ) ? (
                (search ? searchWinnerListByLottery : winnerListByLottery).map(
                  (val, index) => {
