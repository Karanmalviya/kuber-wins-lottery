...
export default function ScratchCardWinner() {
  ...
  useEffect(() => {
    if (scratchCardWinnersData) {
      ...
      setScratchCardWonData(result);
    }
  }, [scratchCardWinnersData, id]);

  const handleSearch = () => {
    const filteredWon = scratchCardWonData
      .filter((item) => (scratchCard ? item.id === +scratchCard : true))
      .map((item) => ({
        ...item,
        wons: item.wons.filter((dateItem) => {
          const selectedDate = scratchCardDate
            ? moment(dateItem.date).format("YYYY-MM-DD")
            : null;
          return !scratchCardDate || selectedDate === scratchCardDate;
        }),
      }))
      .filter((item) => item.wons.length > 0);
    setSearch(true);
    setSearchFilter(filteredWon);
  };
  ...
  return (
    <div style={{backgroundColor: "#f5f6ff"}}>
      ...
      {scratchCardWinnersLoading ? (
        ...
        <div className="container">
          {(
            search ? searchFilter.length > 0 : scratchCardWonData.length > 0
          ) ? (
            (search ? searchFilter : scratchCardWonData)
