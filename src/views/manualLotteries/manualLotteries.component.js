...
export default function ManualLotteriesPage(props) {
                ...
                <nav>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active">
                      <Link to="/manual-draw">Manual Draw</Link>
                    </li>
                  </ol>
                </nav>
              </div>
                ...
                <Row className="row">
                  <Col lg={12}>
                    <Card className="card px-3 pb-3">
                      <h5 className="card-title">Manual Draw</h5>
                      <Form.Select
                        type="text"
                        onChange={(e) => {
                          setSelected({
                    ...
                    {selectLottery && Object.keys(selectLottery).length > 0 && (
                          ...
                          <div>
                            ...
                            <CountDown
                              props={{
                                // type: "ticket",
                                dateTime:
                                  selectLottery.gameDuration +
                                  " " +
                                  selectLottery.startTime,
                              }}
                            />{" "}
                          </div>
                        </div>
                        <div className="text-center">
                          {selectLottery.gameDuration}, {selectLottery.startTime}
                        </div>
                      </Card>
                    )}
                  </Col>
                    ...
                    <Card className="card">
                      <Card.Body className="card-body">
                        <div className="row d-flex align-items-center">
                          <div className="col-6">
                            <h5 className="card-title">Pending Draw Result</h5>
                          </div>
                          <div className="col-6 text-end">
                            <Link
                              className={`btn btn-primary ${
                                !selected?.gameInformationId && `disabled`
                              }`}
                              to={
                                "/manual-draw-history/" +
                                selected?.gameInformationId
                        ...
                        <div className="tableBottomEntries">
                          <h6>
                            Showing{" "}
                            {Object.keys(selectLottery).length
                              ? (page - 1) * limit + 1
                              : 0}{" "}
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  {selected.winnerDetails?.length > 0 &&
                    selected.winnerDetails.map((item, index) => (
                      <>
                        <Row key={index} className="mb-3 mx-2">
                          <Col lg={4} md={4} sm={4}>
                            <Form.Label>Select Customer:</Form.Label>
                            <Form.Select
                              name="winnerUserId"
                              onChange={(e) =>
                                handleManualDrawTickets(index, e)
                              }
                              required
                              value={item.winnerUserId}
                              disabled={data.status}
                            >
                              {manualDrawUsers?.length > 0 &&
                                manualDrawUsers.map((user, i) => (
                                  <Fragment key={i}>
                                    {i === 0 && (
                        ...
                        {selected.winnerDetails.length - 1 === index &&
                          sumOfWinRatio > 100 && (
                            <span className="text-danger">
                              Win Ratio's are not allowed to be more the 100%
                            </span>
                          )}
                      </>
                    ))}
