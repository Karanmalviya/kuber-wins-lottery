...
export default function DepositCommissionLog(props) {
  ...
  const { fetchCommissionUser, count, isLoading, commission } = props;
  ...
  //   if (searchTerm === "" || searchTerm === null) {
  //     filtered = currentCommission;
  //   } else if (/^[a-z\s]+$/i.test(searchTerm)) {
  //     filtered = currentCommission?.filter((row) =>
  //       `${row?.User?.fname} ${row?.User?.lname}`
  //         .toLowerCase()
  //         .includes(searchTerm.toLowerCase())
  //     );
  //   } else if (/^[a-z0-9]+$/i.test(searchTerm)) {
  //     filtered = currentCommission?.filter((row) =>
  //       row?.randomNo?.toLowerCase().includes(searchTerm.toLowerCase())
  //     );
  //   }
                      ...
                      <Card.Body className="card-body">
                        <h5 className="card-title">
                          ...
                          <Link
                            ...
                            // className="btn btn-outline-primary btn-lg-3 ms-2 float-end"
                            className={
                              location.pathname === "/buy-commission"
                            ...
                            <Form>
                              <Form.Group>
                                <Form.Control
                                  ...
                                  placeholder="Trx Id or Usernames"
                                />
                              </Form.Group>
                            </Form>
                          </p>
                                ...
                                commissionData?.map((row, i) => {
                                  return (
                                    <tr key={row.id} className="tableTd">
                                      ...
                                      <td>{row?.level}</td>
                                      <td>
                                        Level {row?.level} Buy commission by{" "}
                                        {row?.UserName}
                                      </td>
                                    </tr>
                                  );
